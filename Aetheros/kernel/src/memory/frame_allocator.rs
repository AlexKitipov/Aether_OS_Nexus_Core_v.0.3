...
const MAX_TRACKED_FRAMES: usize = 1 << 20; // 4 GiB worth of 4KiB frames.
...
impl BootInfoFrameAllocator {
    ...
    /// Build allocator state from bootloader memory map.
    ///
    /// # Safety
    /// `memory_regions` must describe the current machine memory map.
    pub unsafe fn init(
        ...
        // Mark managed range as allocated initially.
        for idx in 0..allocator.frame_count {
            allocator.set_allocated(idx);
        }

        // Free only frames that are explicitly usable.
        for region in memory_regions
            .iter()
            .filter(|r| r.kind == MemoryRegionKind::Usable && r.end > r.start)
        {
    ...
    pub fn deallocate(&mut self, frame: PhysFrame) -> Result<(), FrameAllocatorError> {
    ...
    fn frame_to_index(&self, frame: PhysFrame) -> Result<usize, FrameAllocatorError> {
    ...
    fn index_to_frame(&self, idx: usize) -> PhysFrame {
